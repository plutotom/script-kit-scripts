...
`;
var chatIcon = `<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
...
`;
var commitIcon = `<svg width="20" height="20" viewBox="0 0 8 20" fill="none" xmlns="http://www.w3.org/2000/svg">
...
`;
var openIssuePrIcon = `<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
...
`;
var releaseIcon = `<svg width="20" height="20" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg">
...
`;
var AUTH_TOKEN = await env("GH_CLASSIC_TOKEN", {
  hint: md(`
...
var authorAssociationEmoji = (authorAssociation) => {
...
var notificationReason = await arg("Choose a notification reason:", [
  { name: "🛎 All", value: "all", description: "All notifications" },
  {
    name: "🤹 Assign",
    value: "assign",
    description: "You were assigned to the thread"
  },
  ...
  },
  {
    name: "📨 Invitation",
    value: "invitation",
    description: "You were invited to a repository"
  },
  {
    name: "👨‍🏫 Manual",
    value: "manual",
    description: "You are watching the repository"
  },
  {
    name: "👋 Mention",
    value: "mention",
    description: "You were @mentioned in the thread"
  },
  {
  ...
  },
  {
    ...
    description: "A security alert was published"
  },
  {
    name: "🔁 State Change",
    value: "state_change",
    description: "The thread was marked as read"
  },
  {
    name: "👍 Subscribed",
    value: "subscribed",
    description: "You are subscribed to the thread"
  },
  {
    name: "👨‍👩‍👧‍👦 Team Mention",
    value: "team_mention",
...
var notifications = await octokit.paginate("GET /notifications");
var notifs = await arg(
    ...
    return {
      ...
      preview: async () => {
        ...
        const comments = response.map((comment) => {
          return `## @${comment.user.login} - ${formattedDate(
            comment.updated_at
          )} ${comment.updated_at === comment.created_at ? "" : "🔁"} ${comment.author_association ? `- ${comment.author_association}` : ""} ${authorAssociationEmoji(comment.author_association)} 

 ${comment.body}`;
        }).join("\n\n");
        return md(
          `<h1 style="display:flex;gap:5px;align-items:center;">${renderIcon(
            n.subject.type
          )} ${n.subject.title}</h1> 
