...
const notificationReason = await arg("Choose a notification reason:", [
  ...
  notifications
    ...
    .map((n) => {
      return {
        ...
        value: getUrl(n),
        description: `${n.last_read_at ? "" : "🔔"} ${
          n.subject.title
        } - _${formattedDate(n.updated_at)}_`,
        icon: n.repository.owner.avatar_url,
        preview: async () => {
          const response = await octokit.paginate<IssueComment>(
            ...
            .map((comment) => {
              return `## @${comment.user.login} - ${formattedDate(
                ...
                comment.author_association
                  ? `- ${comment.author_association}`
                  : ""
              } ${authorAssociationEmoji(comment.author_association)} \n\n ${
                comment.body
              }`;
            })
            .join("\n\n");
